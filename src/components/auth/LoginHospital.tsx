...
import { Brain, Building2, Lock, Mail, Eye, EyeOff } from 'lucide-react';
...
export function LoginHospital({ onNavigate }: Props) {
  const [email, setEmail] = useState('admin@cliniquesecure.com');
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center p-6">
      ...
      <Card className="w-full max-w-md mt-20 p-8 bg-white">
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} className="space-y-5">
          ...
          >
            Se connecter à mon espace Hôpital
          </Button>
        </form>
        ...
        <p className="text-center text-gray-600 text-sm mt-6">
          ...
          <button
            onClick={() => onNavigate('signup-hospital-step1')}
        ...
        <div className="mt-8 p-4 bg-blue-50 rounded-lg border border-blue-100">
          <p className="text-xs text-blue-800 text-center">
            🔒 Vos données sont chiffrées et protégées (HDS / RGPD)
          </p>
        </div>
      </Card>
    </div>
